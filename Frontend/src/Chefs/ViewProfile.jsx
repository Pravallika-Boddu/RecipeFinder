...
// Handle OTP sending - Updated to use email
const handleSendOTP = async () => {
  try {
    const response = await axios.post('http://localhost:5000/api/auth/send', {
      email: user.email, // Use the user's email instead of mobile number
      purpose: 'password reset' // Add purpose for the email subject
    });
...
// Handle password update - Updated to use email
const handleUpdatePassword = async () => {
  ...
  try {
    const response = await axios.post('http://localhost:5000/api/auth/verify', {
      email: user.email, // Use email instead of mobileNumber
      otp,
      newPassword,
      confirmPassword,
    });
